  ...
  return (
    <Fragment>
      <li className="arrival-list-item-wrapper">
        ...
        >
          <span className="arrival-list-item__eta">{arrivalMinutes} min</span>
          <span className="arrival-list-item__headsign">{name}</span>
          <span className="arrival-list-item__time">{arrivalString}</span>
        </button>
      </li>
      {expanded ? (
        <li id={contentId} className="follow-list-wrapper">
          {fetcher.state !== "idle" ? (
            <div className="follow-list__loading">Loading…</div>
              ...
              {fetcher.data.arrivals.map((stop, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="follow-list-item__name">
                      {stop.station.name}
                    </span>
